...
Webflow.define('ix', function ($, _) {
  ...
  // Set styles immediately on specified jquery element
  function style(el, data) {
    ...
    // Temporarily override upstream transition
    var upstream = _tram.upstream;
    _tram.upstream = 'none 0s ease 0s';

    // Make sure upstream matches the computed
    if (upstream != computed && !emptyTrans.test(computed)) {
      upstream = computed;
    }

    // Set values and restore upstream value
    _tram.set(tramify(data));
    _tram.upstream = upstream;
  }
